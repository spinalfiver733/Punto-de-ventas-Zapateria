...
const RegistrarVenta = ({ 
  ...
  const handleFinalizarVenta = async () => {
    ...
    try {
      ...
      const response = await api.post('/api/ordenes', ventaData);
      ...
      if (usarSaldoFavor && saldoInfo) {
        await api.put(`/api/saldos/${codigoSaldo}/usar`, {
          FK_VENTA_USO: ventaCreada.PK_VENTA
        });
      }
  
      // Obtener la venta creada
      const ventaResponse = await api.get(`/api/ventas/orden/${response.data.PK_ORDEN}`);
